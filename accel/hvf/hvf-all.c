...
static void do_hv_vm_protect(hwaddr start, uint64_t size,
...
static void hvf_protect_clean_range(hwaddr addr, uint64_t size)
{
    do_hv_vm_protect(addr, size, HV_MEMORY_READ | HV_MEMORY_EXEC);
}

static void hvf_unprotect_dirty_range(hwaddr addr, uint64_t size)
{
